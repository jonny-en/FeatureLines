...
\label{defdem}
Betrachtet man eine 3D-Oberfläche als Terrain mit Bergketten und Tälern, so verlaufen die Demarcating Curves entlang der Hänge, die Täler und Berge trennen. Sie beschreiben dabei die Orte der stärksten Beugung auf der Oberfläche, also die Orte, an denen die Ableitung der Krümmung maximal ist. Diese Punkte auf der Oberfläche zu finden, ist die Aufgabe des Algorithmus.  
\\\\
...
Um die Kurvenpunkte zu finden, wird zunächst für jeden Punkt die Krümmung $\kappa(g_p) = g_p^T\Rmnum{2}g_p^T$ bestimmt. Anschließend werden die Nullstellen von $\kappa(g_p)$ auf der Oberfläche ermittelt.
...
\begin{figure}
	...
	\caption{Eine weiche Stufenkante auf einer Oberfläche. Die Demarcating Curve verläuft orthogonal zu dem Normalschnitt (magenta) in der lokalen Richtung $g_p$ (cyan). \cite{Demarcating}}
	\label{fig:abcd}
\end{figure}
