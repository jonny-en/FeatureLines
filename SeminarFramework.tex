...
\end{equation}
Zu beachten ist hierbei, dass die Normalkrümmung an einem Punkt einer Tangentialrichtung, also der Richtung $v$ des Normalschnitts, zugeordnet wird. Die symmetrische Matrix $\Rmnum{2}$ ist die zweite Fundamentalform (\ref{fundamental}).
...
\label{defdem}
Betrachtet man eine 3D-Oberfläche als Terrain mit Bergketten und Tälern, so verlaufen die Demarcating Curves entlang der Hänge, die Täler und Berge trennen. Sie beschreiben dabei die Orte der stärksten Beugung auf der Oberfläche, also die Orte, an denen die Ableitung der Krümmung maximal ist.
\\
...
\begin{equation}
	...
	\Big]
\end{equation}
$u_1$ und $u_2$ beschreiben hierbei die Hauptrichtungen. Die Ableitung der Krümmung in eine Richtung $\vec{v}$ ergibt sich aus der Multiplikation von $\vec{v}$ mit $C$:
$
C_{ijk} v^i v^j v^k
...
\begin{description}
\item[Krümmungsgradient]\hfill\\
Der Krümmungsgradient ist die Tangentenrichtung der maximalen Abweichung der Normalkrümmung. Diese Richtung maximiert den Ausdruck:
\begin{equation}
g_p= \arg\max_v C_{ijk}v^{i}v^{j}v^{k}, s.t. \Vert v\Vert = 1.
\end{equation}\end{description}
Daraus ergibt sich schlussendlich die Definition eines Kurvenpunktes.
\begin{description}
	\item[Definition.]\hfill \\
	\emph{\textbf{p} ist ein Punkt auf einer Demarcating Curve, wenn gilt:} \begin{equation}
	\kappa(g_p) = g_p^T\Rmnum{2}g_p=0\end{equation}
\end{description}
